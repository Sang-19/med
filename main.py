...
class StatsScreen(MDScreen):
    def __init__(self, **kwargs):
        ...
        try:
            ...
            stats_card.add_widget(detailed_button)
            ...
            # Add a label for detailed analysis
            self.detailed_label = MDLabel(
                text="",
                halign="left",
                theme_text_color="Secondary",
                size_hint_y=None,
            ...
            stats_card.add_widget(self.detailed_label)
...
Keep up the good work!"""
            ...
            self.detailed_label.height = "0dp"  # Reset detailed view
        except Exception as e:
    ...
    def show_detailed_analysis(self, instance):
        try:
            detailed_text = "Detailed Medicine Analysis:\n\n"
            perfect_days = []
            incomplete_days = {}
            
            for key in sorted(store.keys(), reverse=True):  # Sort by date, most recent first
                day_data = store.get(key)
                missed_doses = []
                
                # Check each time slot
                for slot in ["Morning", "Afternoon", "Evening", "Night"]:
                    if not day_data.get(slot, False):
                        missed_doses.append(slot)
                
                # Format the date for display
                display_date = datetime.fromisoformat(key).strftime("%B %d, %Y")
                
                if not missed_doses:
                    perfect_days.append(display_date)
                else:
                    incomplete_days[display_date] = missed_doses
            
            # Add perfect days to the analysis
            if perfect_days:
                detailed_text += "Perfect Days (All 4 doses taken):\n"
                for day in perfect_days[:5]:  # Show last 5 perfect days
                    detailed_text += f"✓ {day}\n"
                if len(perfect_days) > 5:
                    detailed_text += f"...and {len(perfect_days) - 5} more days\n"
            
            # Add incomplete days to the analysis
            if incomplete_days:
                detailed_text += "\nDays with Missed Doses:\n"
                for day, missed in list(incomplete_days.items())[:5]:  # Show last 5 incomplete days
                    detailed_text += f"⚠ {day}: Missed {', '.join(missed)}\n"
                if len(incomplete_days) > 5:
                    detailed_text += f"...and {len(incomplete_days) - 5} more days\n"
            
            self.detailed_label.text = detailed_text
            self.detailed_label.height = "200dp"  # Show the detailed analysis
            
        except Exception as e:
            print(f"Error showing detailed analysis: {e}")
            self.detailed_label.text = "Error loading detailed analysis"
            self.detailed_label.height = "50dp"
